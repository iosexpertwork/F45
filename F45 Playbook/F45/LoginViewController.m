...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if (isFreshDeskOpened==YES)
    {
        //isFreshDeskOpened=NO;
        //[self logout];
    }
}
...
- (void)viewDidLoad {
    ...
    email.translatesAutoresizingMaskIntoConstraints = YES;
    ...
    supportLbl.font=[UIFont fontWithName:PROXIMA_NOVA_BOLD size:25];
...
{
    playBookVC *controller=[self.storyboard instantiateViewControllerWithIdentifier:@"PlayBook"];
...
- (IBAction)login:(id)sender
{
    ...
    else
    {
        if ([Helper isInternetConnected]==YES)
        {
            ...
            NSDictionary *dic=[[NSDictionary alloc]initWithObjects:@[email.text, password.text] forKeys:@[@"email", @"password"]];
            NSLog(@"%@",dic);
    ...
    [delegate mainViewSwitch:controller];
}
